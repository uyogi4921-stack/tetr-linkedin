      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-tetr-gray capitalize">{post.author.role}</p>
        ...
        <div className="mt-3">
          {post.type !== "General" && (
            <span className={`badge ${typeColors[post.type] || "badge-gray"} mb-2`}>{post.type}</span>
          )}
        ...
        <div className="mt-2 flex items-center gap-1 pt-1 border-t border-tetr-border">
          ...
          >
            <Heart className={`w-4 h-4 ${isLiked ? "fill-tetr-green" : ""}`} />
            Like
          </button>
          <button
            onClick={() => setShowComments(!showComments)}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-sm text-tetr-gray hover:bg-tetr-gray-light transition-colors"
          >
            <MessageCircle className="w-4 h-4" />
            Comment
          </button>
          <button className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-sm text-tetr-gray hover:bg-tetr-gray-light transition-colors">
